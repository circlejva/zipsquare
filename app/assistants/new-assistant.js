...
NewAssistant.prototype.btnSearchTapped = function(event){
	if (this.searchModel.value.length > 0){
		this.searchGoogle();	
	}
};
NewAssistant.prototype.searchGoogle = function(){
	toggleLoading("on");
...
NewAssistant.prototype.gotResults = function(transport){
	toggleLoading("off");
	var r = transport.responseJSON.responseData;
	this.resultsModel.items = r.results;
	this.controller.modelChanged(this.resultsModel);
};
NewAssistant.prototype.listTapped = function(event){
...
NewAssistant.prototype.addVenue = function(item){
...
NewAssistant.prototype.addedVenue = function(transport){
	toggleLoading("off");
	banner("Venue " + transport.responseJSON.venue.name + " added!");
	Mojo.Controller.stageController.popScene();
//	var v = transport.responseJSON.venue;
//	var m = v.name + " successfully added! Check-in there?";
//	this.controller.showAlertDialog({
//	    onChoose: function(value) {
//			if(value == 'ok'){
//				this.addVenue(event.item);
...
//		message: m,
//		choices:[
//			{label: $L('Check In'), value:'ok', type:'affirmative'},
//			{label: $L('Not Now'), value:'cancel', type:'color'}    
//		]
//	});	
};
